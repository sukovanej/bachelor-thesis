...
In this chapter, we will discuss problems of relativistic quantum mechanics. We will start
...
we will get a form from which it will be easy to identify the continuity equation $\partialder{\rho}{t} + \nabla \cdot \vec{j} = 0$.
...
\begin{equation*}
    \partialder{\left|\psi\right|^{2}}{t} + \nabla \cdot \bigg[ \frac{\hbar}{2mi} \bigg( \psi \nabla \psi^{*} - \psi^{*} \nabla \psi \bigg) \bigg] = 0
\end{equation*}
...
non-negative for any $\psi: \mathbb{R} \to \mathbb{C}$. In the case of the Klein-Gordon equation, we can see the probability 
...
To solve this problem, Dirac introduced his equation. 

\subsection{Dirac equation}

Let's check the probability density using the procedure of deriving
...
We will use a similar trick we used within the Schrödinger's equation or the Klein-Gordon equation. We
...
\begin{equation*}
    \begin{gathered}
        i \hbar \partial_{\mu} \overline{\psi} \gamma^{\mu} \psi + mc \overline{\psi} \psi = 0 \quad \land \quad
        i \hbar \overline{\psi} \gamma^{\mu} \partial_{\mu} \psi - mc \overline{\psi} \psi = 0 \\
        \implies \partial_{\mu} \overline{\psi} \gamma^{\mu} \psi + \overline{\psi} \gamma^{\mu} \partial_{\mu} \psi = 0
    \end{gathered}
\end{equation*}
...
and thus always a positive quantity! Unfortunately, this doesn't mean the relativistic quantum mechanics is saved.
...
equation describes \textit{zero-spin} particles (thus it can't really describe the electron despite the fact it was
...
principle\footnote{Pauli exclusion principle says that two identical fermions can't occupy the same quantum state.} there
is actually a lower bound energy for the solutions of the Dirac equation. The problem is that this reasoning can't
be applied for the case of the Klein-Gordon equation since bosons ignore the Pauli exclusion principle.
...
of the causality. Causality is the relationship between a cause and an effect. Naively, it feels reasonable to expect
two events to not be causal if they happen in a great distance from each other or within a big time difference. Since
a massive object always travel with a lower speed then a speed of light it makes sense to setup the condition of
causality using the speed of light. Basically, if it is possible for an object with a configuration $X_{1} = (x_{1}, ct_{1})$
to get to the point $X_{2} = (x_{2}, ct_{2})$ then there might be a causality between $X_{1}$ and $X_{2}$.
...
not unexpected in the case of the non-relativistic free particle. We used the definition within the theory of relativity to 
formulate the condition for the causality but the Hamiltonian $\hat{H}$ contains only the lowest order approximation. We could try
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\textwidth]{free_relativistic_particle.png}
...
should be zero. Also for the calculation we will Wick rotate the time using $t' = t_{M} = - i t_{E}$.

\begin{equation*}
    S(p) = - {\hbar} c \sqrt{p^{2} + (mc)^{2}} t_{E} + \frac{i}{\hbar}p \Delta x
\end{equation*}
...
\begin{equation*}
    \begin{gathered}
        \partialder{S(p)}{p} = 0 \\
        p^{2} = \frac{(m c \Delta x)^{2}}{(c t_{E})^{2} + (\Delta x)^{2}}, \ p_{\pm} = \pm \frac{i m c \Delta x}{\sqrt{(\Delta x)^{2} + (c t_{E})^{2}}}
    \end{gathered}
\end{equation*}

We have to make sure whether the values $p_{+}$, $p_{-}$ are saddle points. Let's plug the solution back into $\partialder{S(p)}{p}$.

\begin{equation*}
    \begin{gathered}
        \partialder{S(p)}{p} \bigg|_{p = p_{\pm}} = - c \frac{\pm i \Delta x m c t_{E}}{\sqrt{(c t_{E})^{2} + (\Delta x)^{2}}} 
        \cdot \sqrt{\frac{(c t_{E})^{2} + (\Delta x)^{2}}{(mc^{2} t_{E})^{2}}} + i \Delta x = \mp i \Delta x + i \Delta x
    \end{gathered}
\end{equation*}

From this result we can conclude the only point satisfying the equation $\partialder{S(p)}{p} = 0$ is $p_{+}$. Now we find the value of the 
Hessian $\partialdernd{S(p)}{p}$ in the saddle point $p_{+}$.

\begin{equation*}
    \begin{gathered}
        H = \partialdernd{S(p)}{p} \bigg|_{p = p_{+}} = \frac{\big( (ct_{E})^{2} + (\Delta x)^{2} \big)^{\frac{3}{2}}}{c^{3} m t^{2}}
    \end{gathered}
\end{equation*}

After rotating the time back $t_{E} \to t_{M} = t'$ we can conclude the amplitude $A_{2}$ is

\begin{equation}
    \label{eq:a_2_amplitude}
    A_{2} \approx \bigg| e^{S(p_{+})} \sqrt{\frac{2 \pi}{- H}} \bigg|^{2} = \frac{2 \pi c^{3} m t^{2}}{\big((\Delta x)^{2} - (ct')^{2} \big)^{\frac{3}{2}}} \, e^{- \frac{2mc}{\hbar} \sqrt{(\Delta x)^{2} - (ct')^{2}}} 
\end{equation}

The expression \ref{eq:a_2_amplitude} is non-zero for an arbitrary choice of $\Delta x$ and $t'$ thus it is
non-zero also in the region outside of the light cone. We see we deal with the same problem again.
The difference is that now we deal with an unexplainable problem. We used the relativistic energy and still, the theory
allowed the particle to travel faster than the speed of light with non-zero probability! 
