...
From this result we can conclude the only point satifying the equation $\partialder{S(p)}{p} = 0$ is $p_{+}$. Now we find the value of the 
...
After ratating the time back $t_{E} \to t_{M} = t'$ we can conclude the amplitude $A_{2}$ is

\begin{equation}
    \label{eq:a_2_amplitude}
    A_{2} \approx e^{S(p_{+})} \sqrt{\frac{2 \pi}{- H}} = e^{- \frac{mc}{\hbar} \sqrt{(\Delta x)^{2} - (ct')^{2}}} \sqrt{\frac{2 \pi c^{3} m t^{2}}{\big((\Delta x)^{2} - (ct')^{2} \big)^{\frac{3}{2}}}}
\end{equation}
